...
<div class="md:mx-32 " data-aos="flip-up">
   <div class=" grid xs:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 container justify-center ">
      @foreach($courses as $course)
      <x-trainingcardraj coursename="{{$course->name}}" coursedescription="{{ $course->short_description }}" months="{{$course->courseDuration}}" image="{{asset('uploads')}}/{{$course->image}}" a="{{route('course.inside',$course->id)}}" />
      @endforeach
   </div>
</div>
</div>
